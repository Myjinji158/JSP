...
{
	var url = "";
	url += "/Student/SearchTeacherServlet?search_type=" + $("#search_type").val();
	...
	$.post(url, null, function(rs)
	{
		...
		for ( var i = 0; i < rs.length; i++)
		{
			...
			str += "<td>" + rs[i].id + "</td>";
			str += "<td>" + rs[i].name + "</td>";
			str += "<td>" + rs[i].birth + "</td>";
			str += "<td>" + rs[i].sex + "</td>";
			str += "<td>" + rs[i].major + "</td>";
			str += "<td>" + rs[i].phone + "</td>";
			str += "<td><a href='/Student/EditTeacherServlet?tec_id=" + rs[i].id + "'>编辑</a> / ";
			str += "<a href='/Student/DeleteTeacherServlet?tec_id=" + rs[i].id + "'>删除</a></td>";
			str += "</tr>";
			$("#table").append(str);
		}
	}, "json");
...
{
	var url = "", rt = "";
	url += "/Student/GetTeacherCountPageServlet?search_type=" + $("#search_type").val();
