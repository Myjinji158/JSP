...
function showData(page) {
	var url = "";
	url += "/Student/SearchStudentServlet?search_type="
			+ $("#search_type").val();
	...
	$.post(url, null, function(rs) {
		...
		for ( var i = 0; i < rs.length; i++) {
			...
			str += "<td><img src=" + rs[i].pic
					+ " width='20px' height='20px' /></td>";
			if (roleId == 1) {
				str += "<td><a href='/Student/EditStudentServlet?stu_id="
						+ rs[i].id + "'>编辑</a> / ";
				str += "<a href='/Student/DeleteStudentServlet?stu_id="
						+ rs[i].id + "'>删除</a></td>";
			} else
				str += "<td>&nbsp;/</td>";
			str += "</tr>";
...
function getCountPage() {
	var url = "", rt = "";
	url += "/Student/GetStudentCountPageServlet?search_type="
			+ $("#search_type").val();
