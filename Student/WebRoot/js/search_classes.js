...
function showData(page) {
	var url = "";
	url += "/Student/SearchClassesServlet?search_type="
			+ $("#search_type").val();
	...
	$.post(url, null, function(rs) {
		...
		for ( var i = 0; i < rs.length; i++) {
			...
			str += "<td>" + (i + 1) + "</td>";
			str += "<td>" + rs[i][0].id + "</td>";
			str += "<td>" + rs[i][0].name + "</td>";
			str += "<td>" + rs[i][1].id + "</td>";
			str += "<td>" + rs[i][1].name + "</td>";
			str += "<td>" + rs[i][0].tecName + "</td>";
			str += "<td><a href='/Student/EditClassesServlet?cla_id="
					+ rs[i][0].id + "'>编辑</a> / ";
			str += "<a href='/Student/DeleteClassesServlet?cla_id="
					+ rs[i][0].id + "'>删除</a></td>";
			str += "</tr>";
			$("#table").append(str);
		}
...
function getCountPage() {
	var url = "", rt = "";
	url += "/Student/GetClassesCountPageServlet?search_type="
			+ $("#search_type").val();
